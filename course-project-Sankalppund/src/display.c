...
#warning "gpioEnableDisplay is not implemented, please implement in order to use the display"
	...
	gecko_cmd_hardware_set_soft_timer(32800,TIMER_HANDLE,0);
#else
...
{
	gecko_cmd_hardware_set_soft_timer(NO_OF_TICKS,HANDLE_T,SHOT);
}

void ble_address()
{
	struct gecko_msg_system_get_bt_address_rsp_t *bt_addr = gecko_cmd_system_get_bt_address();
			bd_addr *temp_var = &bt_addr->address;

			sprintf(add_buffer,"%x:%x:%x:%x:%x:%x", temp_var->addr[5],temp_var->addr[4],temp_var->addr[3],temp_var->addr[2],temp_var->addr[1],temp_var->addr[0]);

			displayPrintf(DISPLAY_ROW_BTADDR,add_buffer);
}
